  ...
  CGH.storage = {
    ...
    async saveFile(file) {
      ...
      try {
        ...
        const quota = /quota|full/i.test(String(err?.name || err?.message || err));
        throw new Error(
          quota
            ? "Хранилище браузера заполнено. Очистите очередь или галерею."
            : "Не удалось сохранить файл в очереди"
        );
      }
      const rec = await this.loadBlob("files", id);
      if (!rec) throw new Error("Не удалось сохранить файл в очереди");
      return {
        id,
        name: file.name,
        type: file.type,
